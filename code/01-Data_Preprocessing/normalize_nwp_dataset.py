"""Normalize the standardized NWP cube into train/test NetCDF datasets.
...
For each requested split, normalization parameters are calculated only from
that split. Channels with a mean-map step are centered first, then normalized
with one scalar min and one scalar max over the whole split/channel grid.
"""
...
NORMALIZED_CHANNELS = ["ghi", "sp", "t2m", "tp", "u100", "v100"]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Normalize raw_nwp_standardized.nc into train/test split NetCDF files."
    )
    parser.add_argument(
        "--input",
        type=Path,
...
def normalize_split(
    ...
    output_dir: Path,
    overwrite: bool,
) -> tuple[dict[str, Any], dict[str, xr.DataArray]]:
    ...
    normalized_ds = xr.Dataset(
        ...
        attrs={
            **source_ds.attrs,
            "description": f"Normalized NWP dataset for the {split_name} split.",
            "source_dataset": str(DEFAULT_INPUT_PATH),
            "normalization_split": split_name,
            "normalization_period": f"{start_time} to {end_time}",
            "normalization_note": (
    ...
    output_path = output_dir / f"nwp_normalized_{split_name}.nc"
    print(f"[{split_name}] writing {output_path}", flush=True)
    write_netcdf(normalized_ds, output_path, overwrite)
...
def main() -> None:
    ...
    for split_name, (start_time, end_time) in SPLITS.items():
        split_stats, mean_maps = normalize_split(
            source_ds=source_ds,
    ...
    print(f"Wrote normalized outputs to {args.output_dir}", flush=True)
