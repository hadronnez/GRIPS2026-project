"""Build a 15-minute price-prediction dataset.

The input boundary CSV already contains one row every 15 minutes. This script
keeps only actual-value boundary columns, converts the standardized hourly
price target into the same 15-minute format, then merges both datasets on time.

The output is intended as the first modeling table:

    time, boundary actual features..., price
...
.astype("datetime64[ns]") when an exact datetime64[ns] dtype is needed.
"""
...
FALLBACK_INPUT_PATH = Path("raw_data/targets/preduction.csv")
DEFAULT_PRICE_PATH = Path("output/raw_price_standardized/price.csv")
DEFAULT_OUTPUT_PATH = Path("output/clean_datasets/price_prediction_dataset.csv")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a 15-minute modeling dataset from actual boundary values "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Input 15-minute boundary prediction CSV. Default: "
            f"{DEFAULT_INPUT_PATH}, falling back to {FALLBACK_INPUT_PATH} if needed."
        ),
    )
    parser.add_argument(
        "--price",
    ...
    parser.add_argument(
        ...
        help=(
            "Prefix for hourly-wide price columns, such as A for "
            "A_0min/A_15min/A_30min/A_45min. "
            f"Default: {DEFAULT_PRICE_COLUMN_PREFIX}"
        ),
    )
    parser.add_argument(
        "--start-time",
...
def read_prediction_csv(input_path: Path, time_column: str) -> pd.DataFrame:
    ...
    output = output.rename(
        columns={
            time_column: "time",
            **{column: column.removesuffix(ACTUAL_SUFFIX) for column in actual_columns},
        }
    )
    return output
...
def read_price_csv(price_path: Path, time_column: str, price_column_prefix: str) -> pd.DataFrame:
    ...
    price_columns = [
        f"{price_column_prefix}_{minute}min"
...
def build_price_prediction_dataset(
    input_path: Path,
    price_path: Path,
    output_path: Path,
    time_column: str,
    ...
    features = read_prediction_csv(input_path, time_column)
    validate_15min_time_index(features, "time", start_time, end_time, "Prediction features")
    ...
    print(f"Input features: {input_path}")
    print(f"Price target: {price_path}")
    print(f"Wrote price-prediction dataset: {written_path}")
...
def main() -> None:
    args = parse_args()
    build_price_prediction_dataset(
