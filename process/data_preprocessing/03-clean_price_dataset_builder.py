...
DEFAULT_INPUT_PATH = Path("input/targets/price.csv")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Directory for the output CSV. Default: {DEFAULT_OUTPUT_DIR}",
    )
    parser.add_argument(
        "--time-column",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite an existing price output CSV.",
    )
...
def reshape_hourly(series: pd.Series, column_prefix: str) -> pd.DataFrame:
    working = series.rename(column_prefix).reset_index()
    ...
    hourly = working.pivot(
        index="hour",
        columns="minute",
        values=column_prefix,
    ).reindex(columns=EXPECTED_MINUTES)
    hourly.columns = [f"{column_prefix}_{minute:02d}" for minute in EXPECTED_MINUTES]
...
def write_price_csv(
    ...
    output_path = output_dir / PRICE_OUTPUT_NAME
    ...
    price_to_write.to_csv(output_path, index=False)
    return output_path
...
def build_raw_price_dataset(
    ...
    price_hourly = reshape_hourly(filled, HOURLY_COLUMN_PREFIX)
    validate_hourly_output(price_hourly, start_time, end_time, "price")

    output_path = write_price_csv(price_hourly, output_dir, overwrite)

    print(f"Wrote standardized price output to: {output_path}")
